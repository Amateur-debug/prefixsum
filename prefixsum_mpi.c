...
int main(int argc, char *argv[])
{
    ...
    for (iter = 0; iter < num_iters; iter++) {
        ...
        *buffer = 0;
        ...
        int i;
        for (i = 1; i < my_num_elems; i++)
            local_prefix_sums[i] += local_prefix_sums[i - 1];

        // Perform a binary tree-like communication between processes to propagate prefix sums.
        long received_sum;
        for (i = 1; i < num_procs; i *= 2) {
            if (rank + i < num_procs) {
                MPI_Send(local_prefix_sums + my_num_elems - 1, 1, MPI_LONG, rank + i, 0, MPI_COMM_WORLD);
            }
            if (rank >= i) {
                MPI_Recv(&received_sum, 1, MPI_LONG, rank - i, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                *buffer += received_sum;  
            }   
        }

        // Each process updates its local prefix sums by adding the accumulated prefix sum from other processes.
        if(rank != 0){
            for (i = 0; i < my_num_elems; i++)
                local_prefix_sums[i] += *buffer;
        }  
