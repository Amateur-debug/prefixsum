...
int main(int argc, char *argv[])
{
    ...
    for (iter = 0; iter < num_iters; iter++) {
        ...
        #pragma omp parallel shared(starts, ends, data, prefix_sums, tmp_sums)
        {
            int tid = omp_get_thread_num(); // get the local thread ID
            ...
            // Each thread computes its local prefix sum
            int i;
            for (i = start + 1; i < end; i++)
                prefix_sums[i] += prefix_sums[i - 1];
            tmp_sums[tid] = prefix_sums[end - 1];
        }
       
        // In this serial section, we perform an inclusive prefix sum on the tmp_sums array.
        // This will propagate the sums across all threads.    
        int i;
        for (i = 1; i < num_threads; i++) {
            tmp_sums[i] += tmp_sums[i-1];
        }
        
        // Each thread updates its local prefix sums by adding the accumulated prefix sum from other threads.
        #pragma omp parallel shared(starts, ends, data, prefix_sums, tmp_sums)
        {
            int tid = omp_get_thread_num(); // get the local thread ID
            ...
            if(tid != 0){
                for (i = start; i < end - 1; i++){
                    prefix_sums[i] += tmp_sums[tid - 1];
                }
                prefix_sums[end - 1] = tmp_sums[tid];
            }
        }
        /************************************************************/
        ...
        printf("iteration %d elapsed time: %ld (usec)\n", iter, iter_usec);
        fprintf(fp, "iteration %d elapsed time: %ld (usec)\n", iter, iter_usec);
    }

    // Print timing stats
    printf("Finish OpenMP Parrallel Prefix Sum calculation\n\n");
    fprintf(fp, "Finish OpenMP Parrallel Prefix Sum calculation\n\n");
    printf("Prefix Sum average elapsed time: %ld (usec)\n",
            total_usec / num_iters);
    fprintf(fp, "Prefix Sum average elapsed time: %ld (usec)\n",
            total_usec / num_iters);
